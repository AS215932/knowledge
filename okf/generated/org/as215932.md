---
type: Organization
title: AS215932 / Servify / Hyrule Knowledge Domain
description: Organization-level map of AS215932 repositories, services, infrastructure,
  operational loops, and source-of-truth rules.
resource: https://github.com/AS215932
tags:
- as215932
- hyrule
- knowledge
- organization
- servify
truth_owner: derived
authority: advisory
source_refs:
- repo: AS215932/.github
  path: profile/README.md
  commit: e225c3fe957d598f2e53a52d301730e81ec221d8
  lines: 1-56
  url: https://github.com/AS215932/.github/blob/e225c3fe957d598f2e53a52d301730e81ec221d8/profile/README.md#L1-L56
- repo: AS215932/network-operations
  path: README.md
  commit: 67061d325834a7145252cdf851da1df6a4a38b9e
  lines: 1-207
  url: https://github.com/AS215932/network-operations/blob/67061d325834a7145252cdf851da1df6a4a38b9e/README.md#L1-L207
- repo: AS215932/hyrule-cloud
  path: README.md
  commit: 81e4316f3d1f4e7f770bc2589bd84a2c7972aad5
  lines: 1-180
  url: https://github.com/AS215932/hyrule-cloud/blob/81e4316f3d1f4e7f770bc2589bd84a2c7972aad5/README.md#L1-L180
- repo: AS215932/hyrule-web
  path: README.md
  commit: d94146e67f9eb05f8eeb5c57cab74cbe676f5c79
  lines: 1-17
  url: https://github.com/AS215932/hyrule-web/blob/d94146e67f9eb05f8eeb5c57cab74cbe676f5c79/README.md#L1-L17
- repo: AS215932/noc-agent
  path: README.md
  commit: 98e5010648e34ac0ea6ad8e6a925fef76d0dbea9
  lines: 1-240
  url: https://github.com/AS215932/noc-agent/blob/98e5010648e34ac0ea6ad8e6a925fef76d0dbea9/README.md#L1-L240
- repo: AS215932/hyrule-network-proxy
  path: README.md
  commit: b82dc72bbf382167062bff272606ce84ec20538c
  lines: 1-98
  url: https://github.com/AS215932/hyrule-network-proxy/blob/b82dc72bbf382167062bff272606ce84ec20538c/README.md#L1-L98
- repo: AS215932/hyrule-mcp
  path: README.md
  commit: 326bcc85e1c69f0d7c1839ebaa4abb73acd84185
  lines: 1-177
  url: https://github.com/AS215932/hyrule-mcp/blob/326bcc85e1c69f0d7c1839ebaa4abb73acd84185/README.md#L1-L177
- repo: AS215932/engineering-loop
  path: README.md
  commit: 768cde6c996e42f3f91d395347ba9809e2e020e5
  lines: 1-75
  url: https://github.com/AS215932/engineering-loop/blob/768cde6c996e42f3f91d395347ba9809e2e020e5/README.md#L1-L75
- repo: AS215932/as215932.net
  path: README.md
  commit: 9c0ee2d8f2bb793ea799589ca0fe4f77571b2572
  lines: 1-54
  url: https://github.com/AS215932/as215932.net/blob/9c0ee2d8f2bb793ea799589ca0fe4f77571b2572/README.md#L1-L54
- repo: AS215932/hyrule-business
  commit: 89febc9c95fb6766df071e054cb44fba0b1ec8e4
  url: https://github.com/AS215932/hyrule-business
last_verified_at: '2026-06-17T10:33:31Z'
confidence: high
dispute_policy: repo_wins
repository_count: 10
host_count: 23
---

# Overview

AS215932 is the Servify / Hyrule organization knowledge domain for an IPv6-first ISP, Hyrule Cloud product surface, autonomous NOC tooling, and agentic engineering automation. This concept is generated from organization and repository source material and links to the canonical repo-owned concepts.

# Identity and domains

- `hyrule.host` — customer-facing Hyrule Cloud/product identity.
- `servify.network` — infrastructure identity for nameservers, underlay, management, providers, and internal UIs.
- `as215932.net` — AS215932 overlay/routing identity and public ASN website.

See [AS215932 Domain Policy](../infrastructure/domain-policy.md).

# Repositories

* [AS215932/.github](../repositories/as215932/.github.md)
* [AS215932/network-operations](../repositories/as215932/network-operations.md)
* [AS215932/hyrule-cloud](../repositories/as215932/hyrule-cloud.md)
* [AS215932/hyrule-web](../repositories/as215932/hyrule-web.md)
* [AS215932/noc-agent](../repositories/as215932/noc-agent.md)
* [AS215932/hyrule-network-proxy](../repositories/as215932/hyrule-network-proxy.md)
* [AS215932/hyrule-mcp](../repositories/as215932/hyrule-mcp.md)
* [AS215932/engineering-loop](../repositories/as215932/engineering-loop.md)
* [AS215932/as215932.net](../repositories/as215932/as215932.net.md)
* [AS215932/hyrule-business](../repositories/as215932/hyrule-business.md)

# Production services

* [Hyrule Cloud](../services/hyrule-cloud.md)
* [Hyrule Web](../services/hyrule-web.md)
* [Hyrule NOC Agent](../services/noc-agent.md)
* [Hyrule Network Proxy](../services/hyrule-network-proxy.md)
* [Hyrule MCP](../services/hyrule-mcp.md)
* [Hyrule Engineering Loop](../services/engineering-loop.md)
* [as215932.net](../services/as215932-net.md)

# Projects and strategy

* [AS215932 Network Operations](../projects/network-operations.md)
* [Hyrule Business](../projects/hyrule-business.md)

# Infrastructure footprint

| Asset | Count |
| --- | ---: |
| Repositories indexed | 10 |
| Infrastructure hosts | 23 |
| Routers | 4 |
| API endpoints detected | 306 |
| API schemas detected | 219 |
| Workflows detected | 32 |

## Prefixes

* `as215932_prefix` = `2a0c:b641:b50::/44`
* `customer_subnet` = `2a0c:b641:b51::/48`
* `infra_subnet` = `2a0c:b641:b50:2::/64`
* `router_loopback_subnet` = `2a0c:b641:b50::/64`
* `vpn_clients_subnet` = `2a0c:b641:b50:3::/64`
* `wg_link_prefix` = `2a0c:b641:b50:ff00::/56`

# Operational loops

- [Hyrule NOC Agent](../services/noc-agent.md) handles alert intake, incident investigation, proactive hotspot detection, and human-gated operator decisions.
- [Hyrule MCP](../services/hyrule-mcp.md) provides bounded diagnostic tools for live routing, host, monitoring, DNS, firewall, and packet-path context.
- [Hyrule Engineering Loop](../services/engineering-loop.md) turns approved issues and requests into draft PRs through guarded coding-agent workflows.

# Source-of-truth rules

- Source repositories are canonical for implemented behavior, config, APIs, tests, workflows, deployment pins, and intended infrastructure state.
- Generated OKF concepts are derivative and must cite repo paths/commits.
- OKF-owned curated concepts are advisory/proposed until explicitly reviewed.
- Observed telemetry is evidence only and never silently redefines intended state.

# Key contacts/resources

- NOC: `noc@as215932.net`
- PeeringDB: https://www.peeringdb.com/asn/215932
- Public website: https://as215932.net

# Current gaps/open follow-ups

- Configure `KNOWLEDGE_GH_TOKEN` for scheduled private-repo ingestion.
- Enable branch protection when private branch protection is available for the org/account plan.
- Review proposed curated architecture, policy, lessons, and strategy pages.
- Decide when/where to schedule safe telemetry ingestion beyond manual/local collection.

# Citations

[1] [AS215932/.github:profile/README.md](https://github.com/AS215932/.github/blob/e225c3fe957d598f2e53a52d301730e81ec221d8/profile/README.md#L1-L56)
[2] [AS215932/network-operations:README.md](https://github.com/AS215932/network-operations/blob/67061d325834a7145252cdf851da1df6a4a38b9e/README.md#L1-L207)
[3] [AS215932/hyrule-cloud:README.md](https://github.com/AS215932/hyrule-cloud/blob/81e4316f3d1f4e7f770bc2589bd84a2c7972aad5/README.md#L1-L180)
[4] [AS215932/hyrule-web:README.md](https://github.com/AS215932/hyrule-web/blob/d94146e67f9eb05f8eeb5c57cab74cbe676f5c79/README.md#L1-L17)
[5] [AS215932/noc-agent:README.md](https://github.com/AS215932/noc-agent/blob/98e5010648e34ac0ea6ad8e6a925fef76d0dbea9/README.md#L1-L240)
[6] [AS215932/hyrule-network-proxy:README.md](https://github.com/AS215932/hyrule-network-proxy/blob/b82dc72bbf382167062bff272606ce84ec20538c/README.md#L1-L98)
[7] [AS215932/hyrule-mcp:README.md](https://github.com/AS215932/hyrule-mcp/blob/326bcc85e1c69f0d7c1839ebaa4abb73acd84185/README.md#L1-L177)
[8] [AS215932/engineering-loop:README.md](https://github.com/AS215932/engineering-loop/blob/768cde6c996e42f3f91d395347ba9809e2e020e5/README.md#L1-L75)
[9] [AS215932/as215932.net:README.md](https://github.com/AS215932/as215932.net/blob/9c0ee2d8f2bb793ea799589ca0fe4f77571b2572/README.md#L1-L54)
[10] [AS215932/hyrule-business:repository](https://github.com/AS215932/hyrule-business)
